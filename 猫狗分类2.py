...
#1.数据预处理
train_datagen = ImageDataGenerator(rotation_range=40,       #图像随机旋转的角度范围
                                   width_shift_range=0.2,   #图像在水平方向的平移范围（相对于总宽度的比例）
                                   height_shift_range=0.2,  #图像在竖直方向的平移范围（相对于总高度的比例）
...
history = model.fit_generator(
    train_generator,
    steps_per_epoch=100,#一个epoch抽取100个批量，运行一百次梯度下降进入下一个epoch
    epochs=100,
