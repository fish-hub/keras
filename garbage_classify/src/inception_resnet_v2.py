...
def creat_net(train_generator,validation_generator,batch_size,image_lengh,image_width):
    base_model = applications.InceptionResNetV2(weights='imagenet', include_top=False,input_shape= (image_width,image_lengh,3))
    for layer in base_model.layers:
         layer.trainable = False
    x = base_model.output
    x = tf.keras.layers.Conv2D(128,kernel_size=(3,3),strides=(1,1),activation='relu')(x)
    ...
    early = callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=0, verbose=0, mode='auto', baseline=None, restore_best_weights=False)
    model.fit_generator(train_generator, epochs=30, steps_per_epoch=1707//batch_size,validation_data=validation_generator,
                    validation_steps=264//batch_size, callbacks=[checkpoint,checkpoint])#Reduce])
...
if __name__ =="__main__":
    ...
    #生成图片
    generator_train,generator_test = image_process(batch_size=16,train_dir=train_dir,test_dir=test_dir,image_lengh=image_lengh,image_width=image_width)
    print("图片处理完成")
    #构建网络+训练
    creat_net(generator_train,generator_test,batch_size=16,image_lengh=image_lengh,image_width=image_width)
    print("结束")