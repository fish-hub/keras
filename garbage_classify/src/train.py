...
def creat_net(train_generator,validation_generator,batch_size,image_lengh,image_width):
    #迁移学习
    model = applications.ResNet50V2(weights='imagenet', include_top=False,input_shape= (image_width,image_lengh,3))
    x = tf.keras.layers.Conv2D(128,kernel_size=(3,3),strides=(1,1),activation='relu')(model.output)
    x = tf.keras.layers.Dropout(0.2)(x)
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    x = tf.keras.layers.Dropout(0.2)(x)
    x = tf.keras.layers.Dense(4,activation='softmax')(x)
    myModel = tf.keras.Model(model.input,x)
    #冻结网络层
    ...
    #保存最优模型
    filepath = './模型/ResNet50_V2_weights-improvement-{epoch:02d}-{val_accuracy:.2f}.hdf5'
    ...
    #训练
    myModel.fit_generator(train_generator, epochs=100, steps_per_epoch=1707//batch_size,validation_data=validation_generator,
